...
enum KDCircularProgressGlowMode {
    case forward, reverse, constant, noGlow
}


class KDCircularProgress: UIView, CAAnimationDelegate {
    ...
    var glowAmount: CGFloat = 1.0 {//Between 0 and 1
        didSet {
    ...
    var progressThickness: CGFloat = 0.4 {//Between 0 and 1
        didSet {
    ...
    var trackThickness: CGFloat = 0.5 {//Between 0 and 1
        didSet {
    ...
    var progressInsideFillColor: UIColor? = nil {
        didSet {
    ...
    func animate(fromAngle: Double, toAngle: Double, duration: TimeInterval, relativeDuration: Bool = true) {
        if isAnimating() {
            pauseAnimation()
        }
    ...
    func animate(toAngle: Double) {
        if isAnimating() {
            pauseAnimation()
        }
        animate(fromAngle: angle, toAngle: toAngle, duration: 0.5)
    }
